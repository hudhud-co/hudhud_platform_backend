"""Idempotent JetStream bootstrap for HUDHUD_AUDIT + audit_bridge_entry_v1 only.
...
AUDIT_FILTER = "hudhud.audit.legacy_bridge.observation.audit_entry.v1"
...
async def bootstrap(nats_url: str) -> list[str]:
    ...
    stream_entry = next(
        entry for entry in streams_doc["streams"] if entry["name"] == AUDIT_STREAM
    )
    consumer_entry = next(
        entry
        for entry in consumers_doc["consumers"]
        if entry["durable_name"] == AUDIT_DURABLE
    )
    if consumer_entry["stream"] != AUDIT_STREAM:
        msg = "audit consumer stream binding mismatch in topology"
        raise RuntimeError(msg)
    if consumer_entry["filter_subject"] != AUDIT_FILTER:
        msg = "audit consumer filter binding mismatch in topology"
        raise RuntimeError(msg)
...
def main() -> int:
    ...
    for line in results:
        print(line)
    print("HUDHUD_OBSERVATION_EVENTING_TOPOLOGY_BOOTSTRAPPED")
    return 0
