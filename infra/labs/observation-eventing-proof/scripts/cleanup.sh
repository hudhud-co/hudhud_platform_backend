...
PROJECT="${COMPOSE_PROJECT_NAME:-hudhud-observation-eventing-proof-lab${SUFFIX}}"

docker compose -f "${COMPOSE_FILE}" -p "${PROJECT}" --profile observation-eventing-proof down -v --remove-orphans

docker network inspect hudhud_observation_eventing_proof"${SUFFIX}" >/dev/null 2>&1 && exit 1 || true
docker volume inspect hudhud_observation_eventing_proof_pgdata"${SUFFIX}" >/dev/null 2>&1 && exit 1 || true
docker volume inspect hudhud_observation_eventing_proof_jetstream"${SUFFIX}" >/dev/null 2>&1 && exit 1 || true
