"""Durable inbox deduplication for events Merchant consumes (ADR-0008)."""
...
import pytest
from merchant_fixtures import build_store, inbox_service
...
from merchant.domain.messaging import InboxRecord, InboxStatus

CONSUMER = "merchant_identity_principal_v1"
OTHER_CONSUMER = "merchant_shipment_lifecycle_v1"
...
def _deliver(service: InboxService, calls: list, *, event_id, consumer=CONSUMER):
    return service.handle(
        ...
        event_id=event_id,
        event_type="identity.fact.principal_suspended",
        event_version=1,
        payload={"principal_id": str(uuid4())},
