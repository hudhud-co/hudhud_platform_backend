"""Import boundary guards for the Merchant service.

Merchant talks to Identity over HTTP and to the broker over NATS. It must never reach
...
def test_identity_is_reached_only_over_http() -> None:
    """The one place that may speak to Identity is the introspection adapter."""
    users = sorted(
        str(path.relative_to(SERVICE_ROOT))
        for path in SERVICE_ROOT.rglob("*.py")
        if "httpx" in _imports(path)
    )
    assert users == ["merchant/infrastructure/authorizers/identity.py"]
...
def test_the_domain_layer_has_no_infrastructure_dependency() -> None:
    ...
    offenders = {
        ...
        for path in (SERVICE_ROOT / "merchant" / "domain").rglob("*.py")
...
def test_the_application_layer_does_not_depend_on_the_web_framework() -> None:
    ...
    offenders = {
        ...
        for path in (SERVICE_ROOT / "merchant" / "application").rglob("*.py")
        if _imports(path) & web
    }
    assert offenders == {}


def test_migrations_only_create_merchant_owned_tables() -> None:
    ...
    foreign = sorted(
        name for name in created | dropped if not name.startswith("merchant")
    )
    assert foreign == []
