...
from pickup.config import DEFAULT_OUTBOX_RETRY_BACKOFF_SECONDS
from pickup.domain.entities import OutboxRecord
from pickup.domain.sanitize import sanitize_error_message
from pickup.ports.repository import OutboxRelayStorePort, PublisherPort
...
class OutboxPublisher:
    """Claims and publishes outbox rows — claim commits before broker await."""

    def __init__(
        self,
        *,
        outbox_store: OutboxRelayStorePort,
...
def recover_stale_rows(outbox_store: OutboxRelayStorePort, *, now: datetime) -> int:
