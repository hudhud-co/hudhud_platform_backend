...
def assert_nats_configuration(settings: PickupSettings) -> None:
...
def _has_credentials(settings: PickupSettings) -> bool:
    return bool(
        settings.nats_token
...
class LiveNatsJetStreamClient:
    ...
    def __init__(self, settings: PickupSettings) -> None:
...
def _build_tls_context(settings: PickupSettings) -> ssl.SSLContext:
    if settings.nats_tls_ca_file:
        return ssl.create_default_context(cafile=settings.nats_tls_ca_file)
    return ssl.create_default_context()


def build_live_nats_client(settings: PickupSettings) -> LiveNatsJetStreamClient:
