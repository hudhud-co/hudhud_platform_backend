"""Transport-independent A1 observation consumer coordinator."""
...
from tracking.application.validation import project_timeline_entry, validate_a1_delivery
from tracking.domain.contract import A1_EVENT_TYPE, A1_EVENT_VERSION
from tracking.domain.errors import ContractRejection, PoisonHandlerError, RetryableHandlerError
from tracking.domain.sanitize import sanitize_error_message
from tracking.domain.types import Delivery, InboxRow, ValidatedA1Message
from tracking.ports import (
    ConsumerTransportPort,
    HandleOutcome,
    InboxStorePort,
    TimelineProjectionStorePort,
    UnitOfWorkPort,
)

logger = logging.getLogger("tracking.timeline_consumer")


class TimelineConsumerCoordinator:
    ...
    def __init__(
        ...
        *,
        ...
        observations: TimelineProjectionStorePort,
        ...
        self._inbox = inbox
        self._timeline_entries = observations
    ...
    def handle(self, delivery: Delivery) -> HandleOutcome:
        ...
        try:
            validated = validate_a1_delivery(envelope=envelope, delivery=delivery)
    ...
    def _quarantine_deserialize_poison(self, delivery: Delivery, now: datetime) -> HandleOutcome:
        ...
        return self._quarantine_permanent(
            ...
            event_version=A1_EVENT_VERSION,
            correlation_id=None,
            now=now,
            error_code="DESERIALIZE_FAILURE",
    ...
    def _project(self, validated: ValidatedA1Message, *, received_at: datetime) -> bool:
        observation = project_timeline_entry(validated, received_at=received_at)
        return self._timeline_entries.insert_if_absent(observation)
...
def _poison_delivery_event_id(delivery: Delivery) -> UUID:
    ...
    return uuid5(NAMESPACE_OID, f"tracking-poison:{digest}")
