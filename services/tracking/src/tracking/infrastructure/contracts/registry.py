...
@dataclass(frozen=True, slots=True)
class LoadedA1Registry:
    contracts_root: Path
    contract: A1ContractIdentity
...
@lru_cache(maxsize=1)
def load_a1_registry(*, anchor: Path | None = None) -> LoadedA1Registry:
    ...
    for entry in payload["contracts"]:
        if entry.get("event_type") != "legacy_bridge.observation.shipment_timeline_entry":
            continue
        ...
        contract = A1ContractIdentity(
            ...
            durable_consumer="tracking_bridge_timeline_v1",
        )
        return LoadedA1Registry(contracts_root=contracts_root, contract=contract)
    msg = "registry.yaml missing legacy_bridge.observation.shipment_timeline_entry"
    raise ContractAssetMissingError(msg)


def reset_registry_cache() -> None:
    load_a1_registry.cache_clear()
