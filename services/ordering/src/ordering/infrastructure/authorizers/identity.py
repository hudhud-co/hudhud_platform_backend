"""Ordering authorization adapters.

Same shape as Pickup's and Customer's: default-deny until Identity is configured, then a
real introspection-backed adapter. Ordering never imports Identity — it asks over HTTP.
"""
...
_ROLE_MAP = {
    "CUSTOMER": OrderingRole.CUSTOMER,
    "MERCHANT_OWNER": OrderingRole.MERCHANT_OWNER,
    "MERCHANT_MEMBER": OrderingRole.MERCHANT_MEMBER,
    "SUPPORT": OrderingRole.SUPPORT,
    "OPERATIONS": OrderingRole.OPERATIONS,
}
_REFUSED_STATUSES = frozenset({"SUSPENDED", "CLOSED"})


class DefaultDenyOrderingAuthorizer:
    ...
    async def authorize(
        ...
        *,
        ...
        return OrderingAccessDecision.unauthenticated()


class FakeOrderingAuthorizer:
    """Test-only authorizer resolving a token to a preset actor."""

    def __init__(
        self,
        *,
        token_actors: dict[str, OrderingActor] | None = None,
        denied_commands: frozenset[OrderingCommand] | None = None,
    ...
    async def authorize(
        ...
        *,
        ...
        if command in self._denied:
            return OrderingAccessDecision.forbidden()
        return OrderingAccessDecision.allow(actor)


class IdentityOrderingAuthorizer:
    """Resolve a bearer token into a Ordering actor through Identity introspection."""
    ...
    async def _introspect(self, token: str) -> dict[str, object]:
        ...
        # 403 means Ordering's own service credential was rejected, and 5xx is Identity
...
def build_actor(body: dict[str, object]) -> OrderingActor | None:
...
def _merchant_scopes(body: dict[str, object]) -> frozenset[UUID]:
    """Which merchants this principal speaks for, from Identity's scoped grants.

    Identity does not publish a top-level list of merchant ids: it returns `grants`, each
    with a `scope_kind` and an opaque `scope_id` belonging to another context. A merchant
    owner is a principal holding a `MERCHANT`-scoped grant, and the scope id is the
    merchant.

    Unlike Finance, Ordering keeps **every** merchant scope rather than refusing an
    ambiguous pair: a person may own several stores (Customer App v3 `yourStores`), and
    each Ordering call names the merchant it acts on, so there is nothing to disambiguate.

    An unparseable scope is dropped rather than guessed — attaching an actor to the wrong
    merchant would expose another store's parcels.
    """
    scoped: set[UUID] = set()
