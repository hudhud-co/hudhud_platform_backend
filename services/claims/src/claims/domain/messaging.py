"""Claims's own integration outbox and inbox records (ADR-0008).
...
class InboxRecord:
    """One claims of one event to one consumer.

    Uniqueness is `(consumer_name, event_id)`, not `event_id` alone: two consumers in this
    service must each get their own chance to handle the same message, while a reclaims
