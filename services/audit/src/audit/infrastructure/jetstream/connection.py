...
from audit.config import AuditSettings, RuntimeEnvironment
from audit.domain.contract import A2_DURABLE_CONSUMER, A2_STREAM
from audit.infrastructure.jetstream.binding import (
    ConsumerBindingMismatchError,
    verify_consumer_info,
)

logger = logging.getLogger("audit.jetstream.connection")
...
def build_nats_connect_options(settings: AuditSettings) -> dict[str, Any]:
    ...
    if settings.environment is RuntimeEnvironment.PRODUCTION and not has_credentials:
        msg = "Production NATS requires ADR-0004 credential gate"
    ...
    if settings.environment in {RuntimeEnvironment.LOCAL, RuntimeEnvironment.TEST}:
        if settings.allow_no_auth_local or has_credentials:
            return options
        msg = "Local NATS without auth requires AUDIT_ALLOW_NO_AUTH_LOCAL"
        raise NatsAuthRequiredError(msg)
...
async def bind_existing_pull_consumer(
    ...
    *,
    ...
    if settings.consumer_name != A2_DURABLE_CONSUMER:
        msg = (
            "Configured consumer name does not match A2 durable binding — "
            f"expected {A2_DURABLE_CONSUMER}, got {settings.consumer_name}"
        )
        raise ConsumerBindingMismatchError(msg)
    info = await js.consumer_info(A2_STREAM, A2_DURABLE_CONSUMER)
    verify_consumer_info(info)
    subscription = await js.pull_subscribe_bind(
        durable=A2_DURABLE_CONSUMER,
        stream=A2_STREAM,
    )
...
def _build_tls_context(settings: AuditSettings) -> ssl.SSLContext:
...
def log_connection_failure(exc: BaseException) -> None:
    ...
    if isinstance(exc, ConsumerBindingMismatchError):
        logger.error("audit_nats_binding_mismatch", extra={"error_code": "BINDING_MISMATCH"})
        return
    if isinstance(exc, NatsAuthRequiredError):
        logger.error("audit_nats_auth_required", extra={"error_code": "AUTH_REQUIRED"})
        return
    logger.error(
        "audit_nats_connection_failed",
