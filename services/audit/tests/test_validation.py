"""A2 contract validation unit tests."""
...
from audit.application.validation import validate_a2_delivery
...
def _validate(
    ...
    **delivery_overrides: object,
    ...
    return validate_a2_delivery(envelope=parsed, delivery=delivery)
...
def test_valid_complete_envelope_is_accepted(
    ...
    assert validated.source_table == "audit_logs"  # type: ignore[union-attr]
    assert validated.actor_id is not None  # type: ignore[union-attr]
    assert "delivery_task_id" in validated.safe_metadata  # type: ignore[union-attr]
...
def test_wrong_stream_is_rejected(
    ...
    with pytest.raises(ContractRejection, match="stream"):
        _validate(make_envelope(), make_delivery, stream="HUDHUD_SHIPMENT")
...
def test_wrong_durable_is_rejected(
    ...
    with pytest.raises(ContractRejection, match="durable"):
        _validate(make_envelope(), make_delivery, consumer_name="tracking_bridge_timeline_v1")
