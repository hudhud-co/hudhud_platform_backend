"""Money for the Finance context.

This is the same model the rest of the platform uses, held here rather than imported:
`finance` may not import another service's package, and money is exactly the thing that
must not diverge between them, so the invariants are duplicated deliberately and the
contract tests compare the two.
