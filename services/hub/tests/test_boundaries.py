"""Import boundary guards for the Hub service.

Hub talks to Identity over HTTP and to the broker over NATS. It must never reach
...
from hub.infrastructure.persistence.models import Base
...
def test_identity_is_reached_only_over_http_from_its_adapter() -> None:
    ...
    assert users == ["hub/infrastructure/authorizers/identity.py"]
...
def test_the_domain_layer_has_no_infrastructure_dependency() -> None:
    ...
    offenders = {
        ...
        for path in (SERVICE_ROOT / "hub" / "domain").rglob("*.py")
...
def test_the_application_layer_does_not_depend_on_the_web_framework() -> None:
    ...
    offenders = {
        ...
        for path in (SERVICE_ROOT / "hub" / "application").rglob("*.py")
        if _imports(path) & web
    }
    assert offenders == {}


def test_migrations_only_create_hub_owned_tables() -> None:
    """A migration that touches another service's table is a database-boundary breach.

    Hub's tables are not all prefixed, so the owned set is named explicitly: an
