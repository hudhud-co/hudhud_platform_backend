"""Durable inbox deduplication for events Hub consumes (ADR-0008)."""
...
import pytest
from hub_fixtures import build_store, inbox_service
...
from hub.domain.messaging import InboxRecord, InboxStatus

CONSUMER = "hub_order_registered_v1"
OTHER_CONSUMER = "hub_shipment_custody_v1"
...
def _deliver(service: InboxService, calls: list, *, event_id, consumer=CONSUMER):
    return service.handle(
        ...
        event_id=event_id,
        event_type="order.fact.shipment_registered",
        event_version=1,
