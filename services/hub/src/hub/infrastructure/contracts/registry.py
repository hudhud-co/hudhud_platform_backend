"""CWD-independent canonical contract discovery for Hub-owned events."""
...
DROP_OFF_ACCEPTED_EVENT_TYPE = "hub.fact.drop_off_accepted"
DROP_OFF_ACCEPTED_EVENT_VERSION = 1
PARCEL_HELD_EVENT_TYPE = "hub.fact.parcel_held"
PARCEL_HELD_EVENT_VERSION = 1
...
@dataclass(frozen=True, slots=True)
class LoadedHubFactRegistry:
    contracts_root: Path
    contract: HubFactContract
...
def _build_validator(contract: HubFactContract) -> Draft202012Validator:
...
@lru_cache(maxsize=8)
def load_hub_fact_registry(
    event_type: str,
    event_version: int,
    *,
    anchor: Path | None = None,
) -> LoadedHubFactRegistry:
    """Resolve one registered Hub contract and build its fail-closed validator."""
...
def reset_registry_cache() -> None:
    load_hub_fact_registry.cache_clear()
...
def validate_envelope(
    ...
    loaded = load_hub_fact_registry(event_type, event_version)
