...
def test_compose_uses_dedicated_project_network_and_volumes() -> None:
    ...
    assert base(VOLUME_JS_NAME) in rendered["volumes"]
    assert base(VOLUME_GENERATED_NAME) in rendered["volumes"]
...
def test_cleanup_script_is_executable_and_targets_dedicated_resources() -> None:
    ...
    assert base(VOLUME_JS_NAME) in text
    assert base(VOLUME_GENERATED_NAME) in text
